...
pub fn write_modules<I, S>(path: PathBuf, modules: I) -> io::Result<()>
where
    S: AsRef<str>,
    I: Iterator<Item = S>,
{
...
fn generate_protocol_code(
    ...
    */
    ...
    let enum_name = to_ident(format!("{}Method", struct_name_str));
    ...
    let ts = quote! {
        ...
        impl<T: #trait_name<CI>, CI> #struct_name<T, CI> {
            pub fn new(implementation: T) -> Self { Self(implementation, ::std::marker::PhantomData::default()) }
        }
...
fn to_rust_type_impl(s: &str) -> Option<syn::Path> {
    match s {
        ...
        "byte" => Some(parse_quote!(u8)),
        "uint8" => Some(parse_quote!(u8)),
        ...
        "std_list" => Some(parse_quote!(Vec)),
        "std_map" => Some(parse_quote!(std::collections::HashMap)),
        "qvector" => Some(parse_quote!(Vec)),
