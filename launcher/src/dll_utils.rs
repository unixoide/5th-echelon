...
pub fn parse(data: &[u8]) -> io::Result<Dll> {
    ...
    let version = if let Some(entry) = root_dir
        .id_entries
        .iter()
        .find(|entry| entry.id == RT_VERSION)
    {
        ...
        let mut version_data =
            io::Cursor::new(&rsrc_data[lang_dir.id_entries.first().unwrap().offset..]);
        ...
        assert_eq!(
            ffi::OsString::from_wide(&name).to_str().unwrap(),
            "VS_VERSION_INFO"
        );
