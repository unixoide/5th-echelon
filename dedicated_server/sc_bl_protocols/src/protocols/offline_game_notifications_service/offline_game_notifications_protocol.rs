...
pub struct OfflineGameNotificationsProtocolServer<T: OfflineGameNotificationsProtocolServerTrait<CI>, CI>(T, ::std::marker::PhantomData<CI>);
impl<T: OfflineGameNotificationsProtocolServerTrait<CI>, CI> OfflineGameNotificationsProtocolServer<T, CI> {
    pub fn new(implementation: T) -> Self {
        Self(implementation, ::std::marker::PhantomData)
    }
}
impl<T: OfflineGameNotificationsProtocolServerTrait<CI>, CI> Protocol<CI> for OfflineGameNotificationsProtocolServer<T, CI> {
    fn id(&self) -> u16 {
    ...
    fn handle(
        ...
        match method {
            ...
            Some(OfflineGameNotificationsProtocolMethod::PollSpecificOfflineNotifications) => {
                ...
                debug!(logger, "Request: {:?}", req);
                let resp = self.0.poll_specific_offline_notifications(logger, ctx, ci, req, client_registry, socket);
                debug!(logger, "Response: {:?}", resp);
                Ok(resp?.to_bytes())
            }
            Some(OfflineGameNotificationsProtocolMethod::PollAnyOfflineNotifications) => {
                ...
                debug!(logger, "Request: {:?}", req);
                let resp = self.0.poll_any_offline_notifications(logger, ctx, ci, req, client_registry, socket);
                debug!(logger, "Response: {:?}", resp);
                Ok(resp?.to_bytes())
            }
        }
    ...
    fn method_name(&self, method_id: u32) -> Option<String> {
        OfflineGameNotificationsProtocolMethod::try_from(method_id).ok().map(|e| format!("{:?}", e))
    }
}
...
pub trait OfflineGameNotificationsProtocolServerTrait<CI> {
    fn poll_notifications(
        ...
        warn!(logger, "Method {}.{} not implemented", "OfflineGameNotificationsProtocol", stringify!(poll_notifications));
...
impl<CI> OfflineGameNotificationsProtocolClient<CI> {
    pub fn poll_notifications(&self, logger: &Logger, ctx: &Context, ci: &mut ClientInfo<CI>, request: PollNotificationsRequest) -> Result<PollNotificationsResponse, Error> {
        warn!(logger, "Method {}.{} not implemented", "OfflineGameNotificationsProtocol", stringify!(poll_notifications));
        self.send(logger, ctx, ci, OfflineGameNotificationsProtocolMethod::PollNotifications as u32, request.to_bytes());
        Err(quazal::rmc::Error::UnimplementedMethod)
    }
