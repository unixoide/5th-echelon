                ...
                try:
                    ...
                    if var.lower() == "bits":
                        ...
                        if len(value) > 1 or len(value) <= 0:
                            raise ProtocolException(
                                "FATAL: Invalid value for '%s' option (%s)"
                                % (var, value)
                            )
                        else:
                    ...
                    raise ProtocolException(
                        "FATAL: Invalid options specification (%s)" % opt
                    )
                        ...
                        for i in range(0, lines_to_print):
                            ...
                            if i == central_line:
                                lines.append(
                                    start_line
                                    + str.center(
                                        field_text, (self.bits_per_line * 2) - 1
                                    )
                                    + end_line
                                )
...
class Main:
    ...
    def parse_config_file(self, filename):
        ...
        try:
            ...
            print(
                "Error while reading file %s. Please make sure it exists and it's readable."
                % filename
            )
            sys.exit(1)
            ...
            for i in range(1, len(argv)):
                ...
                if argv[i] == "-f" or argv[i] == "--file":
                    ...
                    if protos <= 0:
                        return (
                            OP_FAILURE,
                            "No protocol specifications found in the supplied file (%s)"
                            % argv[i + 1],
                        )
                ...
                # Bits per line
                    ...
                    try:
                        ...
                        if self.bits_per_line <= 0:
                            return (
                                OP_FAILURE,
                                "Invalid number of bits per line supplied (%s)"
                                % argv[i + 1],
                            )
                    except:
                        return (
                ...
                # Character variations
                    ...
                    if len(argv[i + 1]) != 1:
                        return (
                            OP_FAILURE,
                            "A single character is expected after %s\n%s"
                            % (argv[i], self.get_usage()),
                        )
                    ...
                    # Otherwise, the user meant to display an existing protocol
                        ...
                        # If the specification is ambiguous, we will error
                            ...
                            if len(start_with_the_same) == 1:
                                ...
                                print(
                                    "ERROR: supplied protocol '%s' does not exist."
                                    % argv[i]
                                )
                                sys.exit(1)
                            else:
                                print(
                                    "Ambiguous protocol specifier '%s'. Did you mean any of these?"
                                    % argv[i]
                                )
