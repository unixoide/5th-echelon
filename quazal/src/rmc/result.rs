...
impl From<Error> for u32 {
    fn from(err: Error) -> Self {
        let code = match err {
            Error::Core(inner) => 1 << 16 | u16::from(inner) as u32,
            Error::DDL(inner) => 2 << 16 | u16::from(inner) as u32,
            Error::RendezVous(inner) => 3 << 16 | u16::from(inner) as u32,
            Error::PythonCore(inner) => 4 << 16 | u16::from(inner) as u32,
            Error::Transport(inner) => 5 << 16 | u16::from(inner) as u32,
            Error::DOCore(inner) => 6 << 16 | u16::from(inner) as u32,
            Error::FPD(inner) => 0x65 << 16 | u16::from(inner) as u32,
            Error::Ranking(inner) => 0x67 << 16 | u16::from(inner) as u32,
            Error::Authentication(inner) => 0x68 << 16 | u16::from(inner) as u32,
            Error::DataStore(inner) => 0x69 << 16 | u16::from(inner) as u32,
            Error::ServiceItem(inner) => 0x6c << 16 | u16::from(inner) as u32,
            Error::MatchmakeReferee(inner) => 0x6f << 16 | u16::from(inner) as u32,
            Error::Subscriber(inner) => 0x70 << 16 | u16::from(inner) as u32,
            Error::Ranking2(inner) => 0x71 << 16 | u16::from(inner) as u32,
            Error::SmartDeviceVoiceChat(inner) => 0x72 << 16 | u16::from(inner) as u32,
            Error::Screening(inner) => 0x73 << 16 | u16::from(inner) as u32,
            Error::Custom(inner) => 0x74 << 16 | u16::from(inner) as u32,
            Error::Ess(inner) => 0x75 << 16 | u16::from(inner) as u32,
        };
        code | 0x8000_0000
    }
}
