...
impl Page {
    fn decompress<R: std::io::Read + std::io::Seek>(
        &mut self,
        reader: &mut R,
    ...
    fn read_file(&mut self, entry: &DirEntry) -> std::io::Result<Vec<u8>> {
        ...
        for chunk in &entry.chunks {
            self.seek(std::io::SeekFrom::Start(
                self.data_start + chunk.uncompressed_offset,
            ))?;
            let mut chunk_data = vec![0u8; chunk.chunk_size as usize];
        ...
        match pos {
            ...
            std::io::SeekFrom::End(rel) => {
                self.current_position =
                    (self.header.file_size as u64).overflowing_add_signed(rel).0;
            }
...
impl From<FileSystemEntryBuilder> for FileSystemEntry {
    fn from(value: FileSystemEntryBuilder) -> Self {
        match value {
            FileSystemEntryBuilder::Dir(name, entries) => Self::Dir(
                name,
                entries.into_values().map(|entry| entry.into()).collect(),
            ),
...
impl FileSystemEntry {
                ...
                match component {
                    ...
                    std::path::Component::Normal(os_str) => {
                        ...
                        let entry = entries.entry(name.clone()).or_insert_with(|| {
                            FileSystemEntryBuilder::Dir(name, HashMap::default())
                        });
                        fs_entry = entry;
                    }
                }
    ...
    fn render(&self, ui: &imgui::Ui, open: bool) -> Option<&DirEntry> {
        ...
        match self {
            ...
            FileSystemEntry::File(name, dir_entry) => {
                let clicked = ui.tree_node_config(name).leaf(true).build(|| {
    ...
    fn sort(&mut self) {
        ...
        entries.sort_by(|a, b| match (a, b) {
            ...
            (FileSystemEntry::File(_, _), FileSystemEntry::Dir(_, _)) => {
...
fn load(umd_filepath: PathBuf) -> (String, UmdReader<std::fs::File>, FileSystemEntry) {
    info!("Loading {}", umd_filepath.display());
    let file_name = umd_filepath
        .file_name()
        .unwrap()
        .to_string_lossy()
        .into_owned();

    let mut reader = std::fs::File::open(umd_filepath)
        .and_then(UmdReader::new)
        .unwrap();
...
fn main() {
    ...
    let Some(umd_filepath) = std::env::args()
        .nth(1)
        .map(PathBuf::from)
        .or_else(|| ask_for_filename().unwrap())
    else {
        return;
    };
    let (mut file_name, mut reader, mut fs) = load(umd_filepath);
            ...
            .build(|| {
                ...
                if ui.button("Open Other") {
                    if let Some(path) = ask_for_filename().unwrap() {
                        ...
                        selected_file.take();
                        let (new_file_name, new_reader, new_fs) = load(path);
                ...
                let clicked = ui
                    ...
                    .size([w / 2.0, h - y])
                    .build(|| {
                        let _tbl = ui.begin_table_with_flags(
                            "Table",
                            2,
                ...
                if let Some(entry) = clicked {
                    ...
                    selected_content.replace(content);
                }

                if let Some((dir_entry, content)) =
                    selected_file.as_ref().zip(selected_content.as_ref())
                {
                    ui.child_window("Data")
                        ...
                        .build(|| {
                            ...
                            ui.text(format!(
                                "  Original Offset: {}",
                                dir_entry.chunks[0].original_offset
                            ));
                            ui.text(format!(
                                "  Uncompressed Offset: {} ({})",
                            ...
                            let mut buf = std::str::from_utf8(content)
                                .map(String::from)
                                .ok()
                            ...
                            ui.input_text_multiline("##content", &mut buf, [
                                w - sbsz,
                                h - y - sbsz,
                            ])
